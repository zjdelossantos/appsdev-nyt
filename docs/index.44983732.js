...
const add_breakingNews = (data)=>{
    if (data.length > 0) {
        const imageUrl = data[0].multimedia ? data[0].multimedia[0].url : "default-image.jpg";
...
const add_WNews = (data)=>{
    ...
    data.forEach((element)=>{
        const title = element.title.length < 100 ? element.title : element.title.slice(0, 6) + "...";
        const imageUrl = element.multimedia ? element.multimedia[0].url : "default-image.jpg";
        html += `<div class="news">
                <div class="img">
...
const add_NYTNews = (newsElement, data)=>{
    let html = "";
    data.forEach((element)=>{
        let title = element.title.length < 100 ? element.title : element.title.slice(0, 10) + "...";
        let imageUrl = element.multimedia && element.multimedia.length > 0 ? element.multimedia[0].url : "default-image.jpg";
